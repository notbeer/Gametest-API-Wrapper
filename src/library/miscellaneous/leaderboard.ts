 ...
 */
export function writeLeaderboard([x, y, z]: [number, number, number], objective: Array<string>, displayLength: number, { leaderboardHeading, leaderboardLayout }: { leaderboardHeading?: string, leaderboardLayout?: string } = {}, { compressScore, formatScore }: { compressScore?: boolean, formatScore?: boolean } = {}): any {
    ...
    for(let i = 0; i < onlinePlayers.length; i++) {
        ...
        if(Server.runCommand(`testfor "${onlinePlayers[i]}"`).error) return;
