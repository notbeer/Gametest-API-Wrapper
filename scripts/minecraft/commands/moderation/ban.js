import { Command, Event, Server, Entity, Database, MS } from "../../../library/Minecraft.js";
...
const registerInformation = {
    ...
    example: [
        ...
        'ban "notbeer" 10 hour Bullying player',
...
Command.register(registerInformation, (chatmsg, args) => {
    ...
    db.insertKey(player, banData);
});

Event.on('tick', () => {
    const currentTime = new Date().getTime();
    const bannedPlayers = db.getTableData()?.data;
    if(!bannedPlayers) return;
    for(let key in bannedPlayers) {
        if(bannedPlayers.hasOwnProperty(key) && bannedPlayers[key]?.bannedPlayer) {
            if(bannedPlayers[key]?.unbanTime < currentTime) db.extractKey(key);
