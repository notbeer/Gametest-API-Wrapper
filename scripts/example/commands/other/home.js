...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    if (!args.length || listOptions.includes(args[0])) {
        ...
        if (!args[1])
            return Server.broadcast('§cPlease type a UNIQUE home name to set!', chatmsg.sender.nameTag);
        if (homeName.match(coordFormat))
            return Server.broadcast('§cYou may not indentify your home name in a coordinate format!', chatmsg.sender.nameTag);
        if (data.statusMessage.match(homeRegex))
            return Server.broadcast('§cYou already have a home set with that name!', chatmsg.sender.nameTag);
        ...
        if (!args[1])
            return Server.broadcast('§cPlease type a home name to remove!', chatmsg.sender.nameTag);
        if (!data.statusMessage.match(homeRegex))
            return Server.broadcast("§cYou don't have a home with that name!", chatmsg.sender.nameTag);
        else {
        ...
        if (!args[1])
            return Server.broadcast('§cPlease type a home name to warp to!', chatmsg.sender.nameTag);
        if (!data.statusMessage.match(homeRegex))
            return Server.broadcast("§cYou don't have a home with that name!", chatmsg.sender.nameTag);
        ...
        return Server.broadcast("§cThat's not a valid argument!", chatmsg.sender.nameTag);
});
